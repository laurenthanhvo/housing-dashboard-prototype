...
function numFrom(row, fields) {
  const v = firstValue(row, fields);
  return toNum(v);
}
...
function buildSupplySeries(rows) {
  ...
  rows.forEach(row => {
    ...
    if (!groups.has(id)) groups.set(id, { key, label: String(name), year: Math.trunc(Number(year)), permitted: 0, completed: 0, approved: 0, proposed: 0, adu: 0, jadu: 0, affordable: 0, aboveModerate: 0, points: [] });
...
function addIfNum(obj, field, val) {
  if (isNum(val)) obj[field] += Number(val);
}
...
function combineSeries(a = [], b = []) {
  ...
  [...a, ...b].forEach(row => {
    ...
    if (!isNum(y)) return;
    if (!byYear.has(y)) byYear.set(y, { key: row.key, label: row.label, year: y, permitted: 0, completed: 0, approved: 0, proposed: 0, adu: 0, jadu: 0, affordable: 0, aboveModerate: 0, points: [] });
    const target = byYear.get(y);
    ['permitted', 'completed', 'approved', 'proposed', 'adu', 'jadu', 'affordable', 'aboveModerate'].forEach(f => addIfNum(target, f, row[f]));
    if (row.points?.length) target.points.push(...row.points);
  });
...
function renderTrendChart(id, key) {
  ...
  const data = series.map(d => ({ year: d.year, permitted: d.permitted || 0, completed: d.completed || 0, approved: d.approved || 0 })).filter(d => isNum(d.year));
  const w = el.clientWidth || 300, h = 190, m = { top: 14, right: 12, bottom: 28, left: 38 };
  const maxY = d3.max(data, d => Math.max(d.permitted, d.completed, d.approved)) || 1;
  const x = d3.scalePoint().domain(data.map(d => d.year)).range([m.left, w - m.right]).padding(0.4);
  const y = d3.scaleLinear().domain([0, maxY]).nice().range([h - m.bottom, m.top]);
  const line = field => d3.line().x(d => x(d.year)).y(d => y(d[field]))(data);
  el.innerHTML = `<svg class="chart-svg" width="100%" height="${h}" viewBox="0 0 ${w} ${h}" role="img">
    <g>${y.ticks(4).map(t => `<line x1="${m.left}" x2="${w - m.right}" y1="${y(t)}" y2="${y(t)}" stroke="rgba(23,56,74,.10)"/><text class="axis-label" x="${m.left - 7}" y="${y(t)+4}" text-anchor="end">${compactNumber(t)}</text>`).join('')}</g>
    <path d="${line('approved') || ''}" fill="none" stroke="#D7A64A" stroke-width="3" opacity=".85"/>
    <path d="${line('permitted') || ''}" fill="none" stroke="#B35B3D" stroke-width="3"/>
    <path d="${line('completed') || ''}" fill="none" stroke="#2F7884" stroke-width="3"/>
    ${data.map(d => `<text class="axis-label" x="${x(d.year)}" y="${h - 8}" text-anchor="middle">${String(d.year).slice(-2)}</text>`).join('')}
    <text x="${m.left}" y="14" fill="#B35B3D" font-size="11" font-weight="800">Permitted</text>
    <text x="${m.left + 78}" y="14" fill="#2F7884" font-size="11" font-weight="800">Completed</text>
    <text x="${m.left + 164}" y="14" fill="#D7A64A" font-size="11" font-weight="800">Approved</text>
  </svg>`;
}
